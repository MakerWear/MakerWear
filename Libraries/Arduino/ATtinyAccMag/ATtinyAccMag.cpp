...
// The Arduino two-TinyWireM interface uses a 7-bit number for the address,
...
ATtinyAccMag::ATtinyAccMag(void)
{
  /*
  ...
  */
  m_min = (ATtinyAccMag::vector<int16_t>){-32767, -32767, -32767};
  m_max = (ATtinyAccMag::vector<int16_t>){+32767, +32767, +32767};
...
bool ATtinyAccMag::timeoutOccurred()
{
...
void ATtinyAccMag::setTimeout(unsigned int timeout)
{
  io_timeout = timeout;
}

unsigned int ATtinyAccMag::getTimeout()
{
  return io_timeout;
}

bool ATtinyAccMag::init(deviceType device, sa0State sa0)
{
  // perform auto-detection unless device type and SA0 state were both specified
  if (device == device_auto || sa0 == sa0_auto)
  {
    // check for ATtinyAccMagD if device is unidentified or was specified to be this type
    ...
    // check for ATtinyAccMagDLHC, DLM, DLH if device is still unidentified or was specified to be one of these types
...
/*
Enables the ATtinyAccMag's accelerometer and magnetometer. Also:
- Sets sensor full scales (gain) to default power-on values, which are
  +/- 2 g for accelerometer and +/- 1.3 gauss for magnetometer
  (+/- 4 gauss on ATtinyAccMagD).
- Selects 50 Hz ODR (output data rate) for accelerometer and 7.5 Hz
  ODR for magnetometer (6.25 Hz on ATtinyAccMagD). (These are the ODR
...
*/
void ATtinyAccMag::enableDefault(void)
{
...
void ATtinyAccMag::writeAccReg(byte reg, byte value)
{
...
byte ATtinyAccMag::readAccReg(byte reg)
{
  byte value;
...
void ATtinyAccMag::writeMagReg(byte reg, byte value)
{
...
byte ATtinyAccMag::readMagReg(int reg)
{
  byte value;
  ...
  TinyWireM.send(reg);
  last_status = TinyWireM.endTransmission();
  TinyWireM.requestFrom(mag_address, (byte)1);
  value = TinyWireM.receive();
  TinyWireM.endTransmission();
...
void ATtinyAccMag::writeReg(byte reg, byte value)
{
  // mag address == acc_address for ATtinyAccMagD, so it doesn't really matter which one we use.
...
byte ATtinyAccMag::readReg(int reg)
{
  // mag address == acc_address for ATtinyAccMagD, so it doesn't really matter which one we use.
...
void ATtinyAccMag::readAcc(void)
{
  TinyWireM.beginTransmission(acc_address);
  // assert the MSB of the address to get the accelerometer
  // to do slave-transmit subaddress updating.
  TinyWireM.send(OUT_X_L_A | (1 << 7));
  last_status = TinyWireM.endTransmission();
...
void ATtinyAccMag::readMag(void)
{
  TinyWireM.beginTransmission(mag_address);
  // If ATtinyAccMagD, assert MSB to enable subaddress updating
  // OUT_X_L_M comes first on D, OUT_X_H_M on others
  TinyWireM.send((_device == device_D) ? translated_regs[-OUT_X_L_M] | (1 << 7) : translated_regs[-OUT_X_H_M]);
  last_status = TinyWireM.endTransmission();
  TinyWireM.requestFrom(mag_address, (byte)6);
  ...
  if (_device == device_D)
  {
    ...
    xlm = TinyWireM.receive();
    xhm = TinyWireM.receive();
...
// Reads all 6 channels of the ATtinyAccMag and stores them in the object variables
void ATtinyAccMag::read(void)
{
  readAcc();
  readMag();
}
...
This is the +X axis on the Pololu ATtinyAccMagD carrier and the -Y axis on
the Pololu ATtinyAccMagDLHC, ATtinyAccMagDLM, and ATtinyAccMagDLH carriers.
*/
float ATtinyAccMag::heading(void)
{
  if (_device == device_D)
  {
...
void ATtinyAccMag::vector_normalize(vector<float> *a)
{
...
int ATtinyAccMag::testReg(byte address, regAddr reg)
{
